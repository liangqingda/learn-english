...
每次从记录中抽到题并开始题组时，回复必须先给出仓库级要求的 H4 标题，然后紧接一行斜体来源标注，格式为 `*Picked from xxx*`。`xxx` 必须写明实际抽题来源：普通复习库写 `learning-records/<category>`；已标熟复习写 `familiar-learning-records/<category>`；从合并入口或随机继续复习抽到具体记录后，也按返回记录的实际 `category` 标注，不写合并入口名称。来源标注放在标题之后、练习正文之前。
